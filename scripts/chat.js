...
import { addBook, editBook, deleteBookByTitle, getLocalSuggestion } from "./operations.js";
...
export async function handleLocalCommands(userInput, appendMessage) {
  ...
  if (lowerInput === "help" || lowerInput.includes("tutorial")) {
    const helpText = 
      "Welcome to ShelfSense! Here are some commands you can use:\n" +
      "• add book [Title] by [Author], [Genre], [Rating]\n" +
      "• edit book [Old Title], [New Title], [New Author], [New Genre], [New Rating]\n" +
      "• delete book [Title]\n" +
      "You can also ask questions like 'How do I organize my books?' or 'What does the add book button do?'\n" +
  ...
  if (lowerInput.includes("how do i search") || lowerInput.includes("how to search")) {
    appendMessage("Search functionality is not available. Instead, you can organize your book list by selecting a genre from the dropdown menu.", "bot");
    return true;
  }
  ...
  if (lowerInput.includes("what does") && lowerInput.includes("button") && lowerInput.includes("do")) {
    if (lowerInput.includes("add book")) {
      ...
      appendMessage("I can explain any feature. For example, try asking 'How do I add a new book?'", "bot");
    }
    return true;
  }
